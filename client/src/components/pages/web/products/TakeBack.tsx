...
export default function Takeback({id}: {id: string}) {
  const styles = Styles();
  return (
    <TableContainer id={id} sx={styles.takeBack}>
      <Table>
        <TableHead>
          <TableRow>
            ...
            <TableCell colSpan={3}>
              <Box component={"ol"} sx={{li: {listStyle: "number"}}}>
                ...
                <Box component={"li"}>
                  ...
                  <Box component={"span"}>(단, 상품의 내용을 확인하기 위하여 포장 등을 훼손한 경우는 제외)</Box>
                </Box>
                <Box component={"li"}>
                ...
                <Box component={"li"}>
                  복제가 가능한 상품 등의 포장을 훼손한 경우 <Box component={"span"}>(CD/DVD/GAME/도서의 경우 포장 개봉시)</Box>
                </Box>
              </Box>
            </TableCell>
          </TableRow>
