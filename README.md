...
스페셜티 커피 쇼핑몰 포트폴리오입니다. 원본 `1958shop2`에서 레거시 `frontend` / `frontend-admin`을 제외하고, 이후 샵·관리자·데모 로그인·크라프트 UI 작업을 기능별로 쌓았습니다.
...
cd server
yarn
# config/json/config.json, ormconfig.local.json 을 로컬에서 준비
...
- `frontend/`, `frontend-admin/` (레거시 프론트. `frontend/.env`가 원본에 커밋되어 있었음)
